...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Assumption Budget @if (request()->get('status') == -1)
                            (History)
                        @endif
                    </h1>
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Assumption Budget @if (request()->get('status') == -1)
                                (History)
                            @endif
                        </li>
                    </ol>
            ...
            <div class="d-flex justify-content-end">
                @if (request()->get('status') == -1)
                    <a href="/assumptionbudget" class="btn btn-success">Budget Aktif</a>
                @else
                    @if (((Auth::user()->menu_access->budget ?? 0) & 16) != 0)
                        <a href="/assumptionbudget/monitoring" class="btn btn-warning">Monitoring</a>
                    @endif
                    <a href="/assumptionbudget?status=-1" class="btn btn-info ml-2">History (Budget Non Aktif)</a>
                    <a href="/assumptionbudget/create" class="btn btn-primary ml-2">Tambah Budget Baru</a>
                @endif
            </div>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    @foreach ($budgets as $key => $budget)
                        <tr>
                            ...
                            <td class="text-nowrap">{{ $budget->salespoint->name }}</td>
                            ...
                            @elseif (
                                ($budget->year != $validate_year && $budget->status == 1) ||
                                    ($budget->year != $validate_year && $budget->status == 0))
                                <td>
                                    <a class="btn btn-danger" href="/assumptionbudget/nonActiveBudget/{{ $budget->code }}">
