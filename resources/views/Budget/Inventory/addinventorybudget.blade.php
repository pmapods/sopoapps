...
@section('content')
    ...
    <div class="content-body px-4">
        <form action="/createBudgetRequest/inventory" method="post" id="submitform" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-6 row">
                    <div class="col-4">
                ...
                <div class="col-6 d-flex align-items-center">
                    <div class="form-group mr-2">
                        ...
                        <input type="file" class="form-control-file" name="file" onclick="this.value=null;"
                            placeholder="Pilih File Template Inventory" id="file_template" required accept=".xls, .xlsx" />
                    </div>
                    <div>
                        <a class="btn btn-info mr-2" href='/inventorybudget/create/template'>Download Template</a>
                    </div>
            ...
            <div class="text-center mt-1">
                ...
                <small class="text-danger">* Budget sebelumnya dengan status belum aktif dan aktif di salespoint terkait
                    status akan menjadi Replace / Diganti</small>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#file_template').change(function(evt) {
                ...
                if (files.length > 0) {
                    ...
                    $.ajax({
                        url: '/inventorybudget/create/readtemplate',
                        type: 'POST',
                        data: fd,
                        contentType: false,
                        processData: false,
                        success: function(response) {
                            if (response != 0) {
                                ...
                                if (!error) {
                                    ...
                                    data.forEach(item => {
                                        let append_text = "<tr>";
                                        append_text += '<td>' + item.kode + '</td>';
                                        append_text += '<td>' + item.kategori + '</td>';
                                        append_text += '<td>' + item.keterangan +
                                            '</td>';
        ...
        function showUploadInfo(data, errordata) {
            ...
            let table_append_text =
                '<table class="table table-sm table-bordered small"><thead><tr><th>Nama</th><th>qty</th><th>value</th></thead><tbody>';
            errordata.forEach(item => {
                table_append_text += '<tr><td>' + item.name + '</td><td>' + item.qty + '</td><td>' + item.value +
                    '</td></tr>'
            });
        ...
        function loadItemTracking(budget_upload_id, data) {
            ...
            $('.selected_item').text('Tracking ' + data.code + ' ' + data.keterangan);
